...
func (m *Manager) EnterNamespace(ctx context.Context, env *Environment, cmd []string) error {
	...
	var sshCmd string
	if len(cmd) == 0 {
		// Interactive shell - use su - (login shell) for proper terminal control
		// The user's .bashrc automatically changes to /workspace
		sshCmd = fmt.Sprintf(
			"sudo nsenter --target=$(sudo cat %s) --mount su - %s",
			pidFile,
			env.Name,
		)
	} else {
...
func (m *Manager) createUser(ctx context.Context, username string) error {
	...
	// Create .bashrc that automatically changes to /workspace
	// This allows us to use su - (login shell) which has proper terminal control
	bashrcCmd := fmt.Sprintf("sudo tee /home/%s/.bashrc > /dev/null << 'BASHRC_EOF'\n# Auto-generated by llima-box\n# Change to project workspace on login\nif [ -d /workspace ]; then\n    cd /workspace\nfi\nBASHRC_EOF", username)
	if _, err := m.sshClient.ExecContext(ctx, bashrcCmd); err != nil {
